...
#define NOTIFY_USE_SEPARATOR                                                   \
  false // If true, a separator will be rendered between the title and the
        // content
...
#define NOTIFY_RENDER_OUTSIDE_MAIN_WINDOW                                      \
  true // If true, the notifications will be rendered in the corner of the
       // monitor, otherwise in the corner of the main window
#endif
...
inline void RenderNotifications() {
  ...
  for (size_t i = 0; i < notifications.size(); ++i) {
    ...
    short mainMonitorId =
        static_cast<ImGuiViewportP *>(GetMainViewport())->PlatformMonitor;
    ...
    SetNextWindowPos(
        ImVec2(monitor.WorkPos.x + monitor.WorkSize.x - NOTIFY_PADDING_X,
               monitor.WorkPos.y + monitor.WorkSize.y - NOTIFY_PADDING_Y -
                   height),
    ...
    // Here we render the toast content
    {
      ...
      if (!NOTIFY_NULL_OR_EMPTY(title)) {
        ...
        Text("%s",
             defaultTitle); // Render default title text (ImGuiToastType_Success
                            // -> "Success", etc...)
        wasTitleRendered = true;
      }
